        ...
        foreach ($relationsData as $name => $values) {
            ...
            switch (true) {
                ...
                case $relation instanceof Relations\MorphMany:
				    $first = current($prepared[$name]);
			
                    if(isset($first[static::REMOVE_FLAG_NAME])){
						foreach ($prepared[$name] as $related) {
							...
							/*widia*/
							if(ISSET($related[$keyName."_KEY"])){
								$related[$keyName] = $related[$keyName."_KEY"];
								unset($related[$keyName."_KEY"]);
							}
							if(isset($related[$keyName]) && $related[$keyName] == ""){
								unset($related[$keyName]);
							}
							/*widia*/
						...
						$relation = $this->model()->$name();
						$keyName = $relation->getRelated()->getKeyName();
						get_class($relation->getRelated())::where($this->model()->getKeyName(),$this->model->getKey())->delete();
						foreach ($prepared[$name] as $related) {
							if($related<>""){
								$instance = $relation->make();
								$instance->setAttribute($keyName, $related);
								$instance->setAttribute($this->model()->getKeyName(),$this->model->getKey());
								$instance->save();
							}
						}
					}
    ...
    protected function setFieldValue($id)
    {
        ...
        $this->fields()->each(function (Field $field) use ($data) {
            if (!in_array($field->column(), $this->ignored, true)) {
				/*widia*/
				if (get_class($field) == "Encore\Admin\Form\Field\MultipleSelect") { //widia add
					if(strpos($column=$field->column(),".")>0){
						$rel = explode(".",$column);
						if (method_exists($this->model, $rel[0]) &&
							$this->model->{$rel[0]}() instanceof Relations\Relation
						) {
							$relation =  $this->model->{$rel[0]}()->first();
							if(method_exists($relation, $rel[1]) && $relation->{$rel[1]}() instanceof Relations\Relation)
							{
								$data[$column] = implode(",",$relation->{$rel[1]}()->get('EMP_ID')->modelKeys());
							}
